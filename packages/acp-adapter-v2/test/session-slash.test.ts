...
describe('AcpSession slash routing', () => {
  ...
  it('concatenates text blocks split by the client before slash detection', async () => {
    // Zed splits a slash command and its arguments across multiple
    // ContentBlocks; the args must survive the split instead of being
    // silently dropped from the trailing blocks.
    const sessionId = 'sess-slash-split';
    const { session, calls } = makeFakeSession(sessionId, [endedTurn(sessionId)]);
    const harness = {
      auth: { status: async () => AUTHED_STATUS },
      createSession: async () => session,
    ...
    expect(response.stopReason).toBe('end_turn');
    expect(calls.prompt).toBe(0);
    expect(calls.activate).toEqual([{ name: 'foo', args: 'bar baz qux' }]);
  });

  it('does not treat a prompt whose first block is non-text as a slash command', async () => {
    const sessionId = 'sess-slash-nontext';
    const { session, calls } = makeFakeSession(sessionId, [endedTurn(sessionId)]);
    const harness = {
      auth: { status: async () => AUTHED_STATUS },
      createSession: async () => session,
  ...
  it('intercepts unknown slash commands locally and lets non-slash text flow to Session.prompt', async () => {
    ...
    // the adapter now forwards unknown slash commands to the model as
    // regular prompt text, preserving the full user input.
  ...
  it('intercepts a `/skill:foo` form locally when no skillCommandMap has been seeded', async () => {
    ...
    // prompt text instead of blocking them.
