...
describe('AcpServer session/prompt', () => {
  it('coalesces consecutive AssistantDelta events into one agent_message_chunk and resolves with end_turn', async () => {
    ...
    // Both deltas land in the coalescing buffer and are flushed as a
    // single merged chunk on turn.ended.
    expect(collecting.promptUpdates).toHaveLength(1);
    const only = collecting.promptUpdates[0];
    expect(only?.sessionId).toBe(sessionId);
    expect(only?.update).toMatchObject({
  ...
  it('ignores a subagent turn.ended and resolves on the main agent turn.ended', async () => {
    ...
    expect(response.stopReason).toBe('end_turn');
    await new Promise((resolve) => setTimeout(resolve, 20));
    // Only the main agent's deltas reach the wire, coalesced into one
    // chunk on turn.ended; the subagent's leak events are filtered.
    expect(collecting.promptUpdates).toHaveLength(1);
  ...
  it('flushes a separate chunk per 50ms window when deltas are spaced apart', async () => {
    ...
    const session = {
      ...
      prompt: async (_input: unknown) => {
        for (const fn of listeners) {
          fn({ type: 'assistant.delta', sessionId, agentId: 'main', turnId: 1, delta: 'first' } as Event);
        }
