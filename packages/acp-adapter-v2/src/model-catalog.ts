...
/**
 * Local alias for the provider type — kept here so the v2 package has no
 * direct dependency on `@moonshot-ai/agent-core` (v1). The v2 engine
 * treats provider type as a plain string, but `effectiveModelAlias`
 * (re-exported by the SDK from v1) still expects the v1 union type.
 */
type ProviderType = 'anthropic' | 'vertexai' | 'kimi' | 'openai' | 'google-genai' | 'openai_responses';
...
function providerTypeOf(
  ...
  return (providerType ?? alias.protocol) as ProviderType | undefined;
}
