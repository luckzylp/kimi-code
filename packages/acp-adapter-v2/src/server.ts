...
/**
 * Validate the ACP `additionalDirectories` field from `session/new`,
 * `session/load`, and `session/resume`. Returns the validated array
 * (or `undefined` when the field is absent — the caller normalizes to `[]`).
 *
 * Rules:
 *  - `undefined` → `undefined` (absent = not provided; caller falls back).
...
export function validateAdditionalDirectories(dirs: unknown): string[] | undefined {
  ...
  for (const entry of dirs) {
    if (typeof entry !== 'string' || entry.length === 0 || !path.isAbsolute(entry)) {
      throw RequestError.invalidParams(
        { additionalDirectory: entry },
        'additionalDirectories entries must be non-empty absolute path strings',
      );
    }
  }
