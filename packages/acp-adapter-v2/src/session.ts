    ...
    return new Promise<PromptResponse>((resolve, reject) => {
      ...
      const unsub = this.session.onEvent((event) => {
        ...
        if (event.type === 'assistant.delta') {
          if (!isFromMainAgent(event)) return;
          // Accumulate the delta into the coalescing buffer instead of
          // pushing one `agent_message_chunk` per event: ACP chunks are
        ...
        if (event.type === 'turn.ended') {
          ...
          if (!isFromMainAgent(event)) return;
          settled = true;
          // Force out any text still sitting in the coalescing buffer
          // before the turn resolves — the client must receive the
          // complete message (this mirrors `finalizeAssistantStream` in
          // the TUI, which flushes on turn end).
          flushAssistantTextNow();
...
/**
 * Inspect the `ContentBlock`s of an ACP prompt for a `/skill:<name>`
 * form.  Concatenates all text blocks so that arguments split across
 * multiple blocks (e.g. by the ACP client) are not lost.
